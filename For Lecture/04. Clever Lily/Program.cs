...
namespace CleverLilly
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            double birthdayMoney = 0; // получени пари за четните рожденни дни
            ...
            for (int i = 1; i <= lillysAge; i++)
            {
                ...
                {
                    int money = (i / 2) * 10;
                    birthdayMoney += money; // сумата от пари, получени за четните рожденни дни.
                }
            }

            finalmoney = birthdayMoney + (toysCounter * toysPrice) - (lillysAge - toysCounter);
            // всички пари = парите от четните рожденни дни + (парите от продадени играчки) - (броя пъти, в които брат й е вземал по 1лв).
